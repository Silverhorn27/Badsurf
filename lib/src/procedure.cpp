...
int dc_procedure_open(DC_Procedure *procedure, DC_Dev *dev, DC_ProcedureCtx **ctx_arg, DC_OptionSetting options[]) {
    DC_ProcedureCtx *ctx = (DC_ProcedureCtx*)calloc(1, sizeof(*ctx));
    int i;
    int ret;
    if (!ctx)
        goto fail_ctx;
    ...
    for (i = 0; procedure->options && procedure->options[i].name; i++) {
        ...
        switch (opt->type) {
            case DC_ProcedureOptionType_eInt64:
                ret = sscanf(setting.value, "%ld", (int64_t*)((uint8_t*)ctx->priv + opt->offset));
                break;
    ...
    if (options) {
        ...
        for (arg_i = 0; options[arg_i].name; arg_i++) {
            for (i = 0; procedure->options[i].name; i++) {
                ...
                switch (opt->type) {
                    case DC_ProcedureOptionType_eInt64:
                        ret = sscanf(options[arg_i].value, "%ld", (int64_t*)((uint8_t*)ctx->priv + opt->offset));
                        if (ret != 1)
                            goto fail_priv;
                        break;
                    case DC_ProcedureOptionType_eString:
...
void *dc_procedure_thread_proc(void *packed_args) {
    struct dc_procedure_thread_args_pack *args = (dc_procedure_thread_args_pack*)packed_args;
...
int dc_procedure_perform_loop_detached(DC_ProcedureCtx *ctx, ProcedureDetachedLoopCB callback,
    ...
    int r;
    struct dc_procedure_thread_args_pack *args = (dc_procedure_thread_args_pack*)calloc(1, sizeof(*args));
