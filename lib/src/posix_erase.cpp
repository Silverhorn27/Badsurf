...
typedef struct posix_erase_priv PosixErasePriv;
...
static int Open(DC_ProcedureCtx *ctx) {
    int r;
    PosixErasePriv *priv = (PosixErasePriv*)ctx->priv;
...
static DC_ProcedureOption options[] = {
    { "start_lba", "set LBA address to begin from", offsetof(PosixErasePriv, start_lba), DC_ProcedureOptionType_eInt64 },
    { NULL }
};
...
DC_Procedure posix_write_zeros = {
    ...
    .perform = Perform,
    .close = Close,
    .priv_data_size = sizeof(PosixErasePriv),
    .options = options,
};
